 ...
 * @throw nts::Exception::InvalidTypeException if the component is not of a
 * valid type
 */
void nts::Circuit::setLink(std::size_t pin, nts::IComponent &other,
                           std::size_t otherPin) {
  if (dynamic_cast<nts::IOComponent *>(&other) != nullptr)
    throw nts::Exception::InvalidTypeException(
        "other must not be an IOComponent");
}
 ...
 * valid type
 */
void nts::Circuit::setPin(std::size_t pin, nts::IComponent &other,
                          std::size_t otherPin) {
  if (dynamic_cast<nts::IOComponent *>(&other) != nullptr)
    throw nts::Exception::InvalidTypeException(
        "other must not be an IOComponent");
